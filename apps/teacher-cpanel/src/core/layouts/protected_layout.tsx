...
export default function ProtectedLayout() {
  ...
  if (isLoading || assistantLoading) return <div className="flex min-h-screen items-center justify-center bg-canvas text-[12px] text-text-muted">جاري التحقق من الحساب...</div>
  if (assistantMode) {
    if (assistantError || !assistant) return <Navigate to={`/login?mode=assistant&redirect=${encodeURIComponent(location.pathname)}`} replace />
    return <Outlet />
  }
  if (isError || !teacher) return <Navigate to={`/login?redirect=${encodeURIComponent(location.pathname)}`} replace />

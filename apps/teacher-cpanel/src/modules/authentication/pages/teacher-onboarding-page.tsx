...
import { Button, Card, Input, Select } from '@portal-edu/ui'
import { z } from 'zod'
import { useNotification } from '@/core/hooks/use_notification'
import { useTeacherMe, useTeacherSubjects } from '../api/auth.queries'
import { useCompleteTeacherOnboarding, type TeacherOnboardingPayload } from '../api/auth.mutations'
...
type SelectedStage = { stageGroup: 'primary' | 'preparatory' | 'secondary'; gradeNumber: number }
...
function renderPolicy(content: string) {
  return content.split(/\n\n+/).map((block, index) => {
    ...
    return (
      <section key={index} className="border-b border-border-subtle pb-4 last:border-0">
        <h3 className="text-[13px] font-bold text-text">{heading?.[1] || 'معلومات مهمة'}</h3>
        {text && (
          <p className="mt-1 whitespace-pre-line text-[12px] leading-7 text-text-muted">{text}</p>
        )}
      </section>
    )
  })
...
export default function TeacherOnboardingPage() {
  ...
  const { data: teacher, isLoading: teacherLoading, isError: teacherError } = useTeacherMe()
  ...
  const [editingField, setEditingField] = useState<'fullName' | 'phoneNumber' | 'subjectSpecialization' | null>(null)
  ...
  useEffect(() => {
    if (teacher) reset({ fullName: teacher.fullName, phoneNumber: teacher.phoneNumber || '', subjectSpecialization: teacher.subjectSpecialization || '' })
  }, [reset, teacher])

  useEffect(() => {
    if (teacher && !teacher.onboardingRequired) navigate('/dashboard', { replace: true })
    if (teacherError) navigate('/login', { replace: true })
  }, [navigate, teacher, teacherError])

  const toggleStage = (stageGroup: SelectedStage['stageGroup'], gradeNumber: number) =>
    setSelectedStages((current) =>
      current.some((item) => item.stageGroup === stageGroup && item.gradeNumber === gradeNumber)
        ? current.filter(
            (item) => !(item.stageGroup === stageGroup && item.gradeNumber === gradeNumber)
          )
  ...
  const beginEdit = (field: 'fullName' | 'phoneNumber' | 'subjectSpecialization') => {
    setEditingField(field)
    setEditingValue(getValues(field) || '')
  }
  const saveEdit = () => {
    if (!editingField) return
    setValue(editingField, editingValue, { shouldValidate: true, shouldDirty: true })
    setEditingField(null)
  }
  const finish = async (values: Values) => {
    if (!policy || !accepted) return notify.error('يجب قراءة الشروط والموافقة عليها')
    ...
    try {
      ...
      notify.error(error instanceof Error ? error.message : 'تعذر إكمال الإعداد')
    }
  }
  if (teacherLoading || policyLoading || subjectsLoading || !teacher || !policy || !subjects)
    return (
  ...
  return (
    <div className="min-h-screen bg-canvas" dir="rtl">
      ...
      <main className="mx-auto max-w-3xl px-5 py-10 sm:py-14">
        <div className="mb-8">
          <p className="text-[11px] font-semibold text-primary">مرحباً بك، {teacher.fullName}</p>
          <h1 className="mt-2 text-2xl font-bold tracking-tight text-text">لنجهّز حسابك</h1>
        ...
        <div className="mb-7 flex items-center gap-2 overflow-x-auto pb-1">
          {steps.map((label, index) => (
            <div key={label} className="flex shrink-0 items-center gap-2">
              ...
              {index < steps.length - 1 && <span className="mx-1 h-px w-5 bg-border sm:w-12" />}
            </div>
          ))}
        </div>
                  ...
                  {STAGE_OPTIONS.map((stage) => (
                    ...
                    >
                      <div className="flex items-start gap-3">
                        ...
                        <div>
                          <h3 className="text-[13px] font-bold text-text">{stage.label}</h3>
                          <p className="mt-0.5 text-[11px] text-text-muted">{stage.description}</p>
                        </div>
                      </div>
                      <div className="mt-3 flex flex-wrap gap-2">
                        {stage.grades.map((grade) => {
                          const selected = selectedStages.some(
                            (item) =>
                              item.stageGroup === stage.stageGroup && item.gradeNumber === grade
                          )
                          return (
                            <button
                              key={grade}
                              type="button"
                              onClick={() => toggleStage(stage.stageGroup, grade)}
                            ...
                            >
                                {selected && <Check size={11} strokeWidth={3} className={`${selected && 'text-primary'}`} />}
            ...
            {step === 2 && (
              ...
              >
                <div className="divide-y divide-border-subtle rounded-sm border border-border-subtle">
                  <EditableReview label="الاسم" field="fullName" editingField={editingField} editingValue={editingValue} value={getValues('fullName')} onEdit={beginEdit} onChange={setEditingValue} onSave={saveEdit} onCancel={() => setEditingField(null)} />
                  <EditableReview label="الهاتف" field="phoneNumber" editingField={editingField} editingValue={editingValue} value={getValues('phoneNumber') || 'لم تتم إضافته'} onEdit={beginEdit} onChange={setEditingValue} onSave={saveEdit} onCancel={() => setEditingField(null)} />
                  <SubjectReview selectedSubjectId={selectedSubjectId} subjects={subjects} onChange={(value) => { setSelectedSubjectId(value); setValue('subjectSpecialization', subjects.find((subject) => String(subject.id) === value)?.name || '') }} />
                  <div className="p-3">
  ...
  return (
    <div className="flex items-center justify-between gap-4 p-3">
      ...
      {editing ? (
        ...
        <button type="button" className="group flex items-center gap-2 text-[12px] font-semibold text-text" onClick={() => onEdit(field)}>
          <span>{value}</span>
          <Pencil size={12} className="text-text-muted opacity-0 transition-opacity group-hover:opacity-100" />
        </button>
      )}
    </div>
  )
...
function SubjectReview({ selectedSubjectId, subjects, onChange }: { selectedSubjectId: string; subjects: Array<{ id: number; name: string; icon: string }>; onChange: (value: string) => void }) {
  return (
    <div className="flex items-center justify-between gap-4 p-3">
      <span className="text-[10px] text-text-muted">المادة</span>
      <div className="flex min-w-fit justify-start items-center gap-2">
        <Select value={selectedSubjectId} onChange={onChange} options={subjects.map((subject) => ({ value: String(subject.id), label: subject.name, icon: <SubjectIcon icon={subject.icon} className="h-3.5 w-3.5" /> }))} size="sm" />
      </div>
    </div>
  )
}

...
export function TimeRangePicker({ 
  ...
  return (
    <div className="flex flex-col gap-1">
      {label && <label htmlFor={id} className={cn('w-fit text-[13px] font-bold', error ? 'text-danger' : 'text-text')}>{label}</label>}
      ...
      >
        <div className="flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-2 pt-3 border-t border-border-subtle">
            {rangeInvalid && (
              <p className="m-0 text-[11px] font-bold text-center text-danger bg-danger-subtle py-1 rounded-xs">
      ...
      {error ? <p className="text-[11px] font-medium text-danger mt-1">{error}</p> : hint ? <p className="text-[11px] text-text-muted mt-1">{hint}</p> : null}
    </div>
  )
}
...
import { Bell, Maximize, Menu, Minimize, PanelRightClose, PanelRightOpen } from 'lucide-react'
  ...
  return (
    <div className="flex h-screen w-full overflow-hidden bg-secondary-hover text-ink" dir="rtl">
      ...
      >
        <div
          className={cn(
            'relative flex min-w-0 flex-1 flex-col overflow-hidden bg-surface transition-[border-radius] duration-300 ease-in-out',
            isFullscreen ? 'rounded-none' : 'rounded-md'
          )}
        >
          <main className="flex-1 overflow-y-auto scroll-smooth bg-canvas p-4 md:p-4">
            {navbar}
            {children}
          </main>
        </div>
...
export function LayoutNavbar({
  ...
  return (
    <header className="sticky top-0 z-30 mx-auto mb-4 flex shrink-0 items-center justify-between bg-canvas pb-2 backdrop-blur-md">
      <div className="flex items-center gap-3">
        ...
        <h1 className="text-[16px] font-bold tracking-tight text-text">{title}</h1>
      </div>
